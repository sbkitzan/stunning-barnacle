...
SELECT * FROM marvels WHERE gender = "Female" AND alignment = "Good";

SELECT * FROM marvels ORDER BY height_m;


SELECT AVG(intelligence), gender FROM marvels GROUP BY gender;

SELECT AVG(height_m), gender FROM marvels GROUP BY gender;

SELECT MAX(height_m), gender FROM marvels GROUP BY gender;

SELECT name,
    CASE
        WHEN height_m < 1.70 AND gender = "Female" THEN "short"
        WHEN height_m < 1.80 AND gender = "Female" THEN "average"
        WHEN height_m >= 1.80 AND gender = "Female" THEN "tall"
        WHEN height_m < 1.90 AND gender = "Male" THEN "short"
        WHEN height_m < 2.10 AND gender = "Male" THEN "average"
        WHEN height_m >=2.10 AND gender = "Male" THEN "tall"
    END as "height_classification"
FROM marvels;

SELECT name, alignment, intelligence FROM marvels GROUP BY name HAVING intelligence > 4 ORDER BY intelligence;
